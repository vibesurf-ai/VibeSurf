...
Section "Main Application" SecMain
    ...
    FileWrite $0 "from datetime import datetime, timezone$\r$\n"
    FileWrite $0 "$\r$\n"
    FileWrite $0 "def detect_china_region():$\r$\n"
    FileWrite $0 "    '''Detect if the user is in China region'''$\r$\n"
    FileWrite $0 "    print('[Region Detection] Checking location...')$\r$\n"
    FileWrite $0 "    # Try IP location check first$\r$\n"
    FileWrite $0 "    ip_result = check_ip_location()$\r$\n"
    FileWrite $0 "    if ip_result == 'China':$\r$\n"
    FileWrite $0 "        print('[Region Detection] Detected: China (via IP)')$\r$\n"
    FileWrite $0 "        return True$\r$\n"
    FileWrite $0 "    elif ip_result == 'NotChina':$\r$\n"
    FileWrite $0 "        print('[Region Detection] Detected: International (via IP)')$\r$\n"
    FileWrite $0 "        return False$\r$\n"
    FileWrite $0 "    # Fallback to system locale check$\r$\n"
    FileWrite $0 "    locale_result = check_system_locale()$\r$\n"
    FileWrite $0 "    if locale_result:$\r$\n"
    FileWrite $0 "        print('[Region Detection] Detected: China (via Timezone UTC+8)')$\r$\n"
    FileWrite $0 "    else:$\r$\n"
    FileWrite $0 "        print('[Region Detection] Detected: International (via Timezone)')$\r$\n"
    FileWrite $0 "    return locale_result$\r$\n"
    FileWrite $0 "$\r$\n"
    FileWrite $0 "def check_ip_location():$\r$\n"
    FileWrite $0 "    '''Check IP location using public APIs'''$\r$\n"
    FileWrite $0 "    # Try ipinfo.io$\r$\n"
    FileWrite $0 "    try:$\r$\n"
    FileWrite $0 "        with urllib.request.urlopen('https://ipinfo.io/json', timeout=5) as response:$\r$\n"
    FileWrite $0 "            data = json.loads(response.read().decode('utf-8'))$\r$\n"
    FileWrite $0 "            if data.get('country') == 'CN':$\r$\n"
    FileWrite $0 "                return 'China'$\r$\n"
    FileWrite $0 "            elif data.get('country'):$\r$\n"
    FileWrite $0 "                return 'NotChina'$\r$\n"
    FileWrite $0 "    except Exception:$\r$\n"
    FileWrite $0 "        pass$\r$\n"
    FileWrite $0 "    $\r$\n"
    FileWrite $0 "    # Try api.ip.sb$\r$\n"
    FileWrite $0 "    try:$\r$\n"
    FileWrite $0 "        with urllib.request.urlopen('https://api.ip.sb/geoip', timeout=5) as response:$\r$\n"
    FileWrite $0 "            data = json.loads(response.read().decode('utf-8'))$\r$\n"
    FileWrite $0 "            if data.get('country_code') == 'CN':$\r$\n"
    FileWrite $0 "                return 'China'$\r$\n"
    FileWrite $0 "            elif data.get('country_code'):$\r$\n"
    FileWrite $0 "                return 'NotChina'$\r$\n"
    FileWrite $0 "    except Exception:$\r$\n"
    FileWrite $0 "        pass$\r$\n"
    FileWrite $0 "    $\r$\n"
    FileWrite $0 "    return 'Unknown'$\r$\n"
    FileWrite $0 "$\r$\n"
    FileWrite $0 "def check_system_locale():$\r$\n"
    FileWrite $0 "    '''Check if system locale indicates China region'''$\r$\n"
    FileWrite $0 "    try:$\r$\n"
    FileWrite $0 "        # Check timezone$\r$\n"
    FileWrite $0 "        utc_offset = -timezone.utc.utcoffset(datetime.now()) // 3600$\r$\n"
    FileWrite $0 "        # China is UTC+8$\r$\n"
    FileWrite $0 "        if utc_offset == 8:$\r$\n"
    FileWrite $0 "            return True$\r$\n"
    FileWrite $0 "    except Exception:$\r$\n"
    FileWrite $0 "        pass$\r$\n"
    FileWrite $0 "    return False$\r$\n"
    FileWrite $0 "$\r$\n"
    FileWrite $0 "def check_latest_vibesurf_version(use_china_mirror=False):$\r$\n"
    FileWrite $0 "    # Try China mirror first if in China region$\r$\n"
    FileWrite $0 "    if use_china_mirror:$\r$\n"
    FileWrite $0 "        try:$\r$\n"
    FileWrite $0 "            print('[Version Check] Using Aliyun mirror API...')$\r$\n"
    FileWrite $0 "            with urllib.request.urlopen('https://mirrors.aliyun.com/pypi/web/json/vibesurf', timeout=3) as response:$\r$\n"
    FileWrite $0 "                data = json.loads(response.read().decode('utf-8'))$\r$\n"
    FileWrite $0 "                return data['info']['version']$\r$\n"
    FileWrite $0 "        except Exception:$\r$\n"
    FileWrite $0 "            print('[Version Check] Aliyun mirror failed, trying PyPI...')$\r$\n"
    FileWrite $0 "    $\r$\n"
    FileWrite $0 "    # Fallback to official PyPI$\r$\n"
    FileWrite $0 "    try:$\r$\n"
    ...
    FileWrite $0 "        $\r$\n"
    FileWrite $0 "        # Detect region for mirror selection$\r$\n"
    FileWrite $0 "        use_china_mirror = detect_china_region()$\r$\n"
    FileWrite $0 "        print(f'[Mirror Selection] Will use {'Aliyun' if use_china_mirror else 'PyPI'} mirror')$\r$\n"
    FileWrite $0 "        $\r$\n"
    ...
    FileWrite $0 "        latest_version = check_latest_vibesurf_version(use_china_mirror)$\r$\n"
    FileWrite $0 "        $\r$\n"
    ...
    FileWrite $0 "            print('Updating VibeSurf...')$\r$\n"
    FileWrite $0 "            $\r$\n"
    ...
    FileWrite $0 "            if use_china_mirror:$\r$\n"
    FileWrite $0 "                upgrade_cmd = ['uv', 'pip', 'install', 'vibesurf', '-U', '--python', venv_python_path, '--index-url', 'https://mirrors.aliyun.com/pypi/simple/']$\r$\n"
    FileWrite $0 "            else:$\r$\n"
    ...
    FileWrite $0 "                    if use_china_mirror:$\r$\n"
    FileWrite $0 "                        upgrade_cmd_fallback = ['uv', 'pip', 'install', 'vibesurf', '-U', '--index-url', 'https://mirrors.aliyun.com/pypi/simple/']$\r$\n"
    FileWrite $0 "                    else:$\r$\n"
    ...
    FileWrite $0 "                    if use_china_mirror:$\r$\n"
    FileWrite $0 "                        upgrade_cmd = [pip_cmd, 'install', 'vibesurf', '-U', '-i', 'https://mirrors.aliyun.com/pypi/simple/']$\r$\n"
    FileWrite $0 "                    else:$\r$\n"
    ...
    ; Detect region and use appropriate mirror
    DetailPrint "Detecting installation region..."
    nsExec::ExecToLog 'powershell -NoProfile -Command "try { $tz = Get-TimeZone; if ($tz.BaseUtcOffset.TotalHours -eq 8) { exit 1 } } catch { }; exit 0"'
    Pop $8
    ${If} $8 == 1
        DetailPrint "China region detected, using Aliyun mirror for faster downloads..."
        FileWrite $3 "Region: China - Using Aliyun mirror$\r$\n"
        nsExec::ExecToLog 'cmd /c "cd /d "$INSTDIR" && uv pip install vibesurf pyinstaller setuptools wheel pip -U --index-url https://mirrors.aliyun.com/pypi/simple/"'
    ${Else}
        DetailPrint "Using default PyPI mirror..."
        FileWrite $3 "Region: International - Using default PyPI$\r$\n"
