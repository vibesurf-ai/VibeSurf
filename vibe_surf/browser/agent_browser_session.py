    ...
    async def connect(self, cdp_url: str | None = None) -> Self:
        ...
        try:
            ...
            if self.agent_focus:
                ...
                # Import welcome modal content with language detection
                from vibe_surf.browser.welcome_modal_content import get_welcome_js

                # Get the appropriate welcome modal based on IP location detection
                # China (CN) -> Chinese, all other countries -> English
                welcome_js = await get_welcome_js(extension_path=extension_path)
