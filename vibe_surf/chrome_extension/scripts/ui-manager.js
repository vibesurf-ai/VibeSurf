...
class VibeSurfUIManager {
  ...
  initializeManagers() {
    try {
      ...
      // Initialize settings workflow
      this.settingsWorkflow = new VibeSurfSettingsWorkflow(this, this.apiClient);
  ...
  async handleSendTask() {
    ...
    try {
      ...
      const selectedTabsData = this.settingsWorkflow?.getSelectedTabsForTask();
      if (selectedTabsData) {
        taskData.selected_tabs = selectedTabsData;
      }
      
      // Add selected skills information if any
      const selectedSkillsData = this.settingsWorkflow?.getSelectedSkillsForTask();
  ...
  handleTaskInputKeydown(event) {
    ...
    if (event.key === 'Backspace' || event.key === 'Delete') {
      if (this.settingsWorkflow?.handleTabTokenDeletion(event)) {
        event.preventDefault();
        return;
      }
      // Also handle skill token deletion
      if (this.settingsWorkflow?.handleSkillTokenDeletion(event)) {
        event.preventDefault();
        return;
      }
    }
    ...
    if (event.key === 'Tab' && this.settingsWorkflow?.skillSelectorState?.isVisible) {
      if (this.settingsWorkflow.skillSelectorState.filteredSkills?.length === 1) {
        event.preventDefault();
        this.settingsWorkflow.selectSkill(this.settingsWorkflow.skillSelectorState.filteredSkills[0]);
      }
    }
  }
  ...
  handleTaskInputChange(event) {
    ...
    this.settingsWorkflow?.handleTabSelectorInput(event);
    
    // Check for / character to trigger skill selector
    this.settingsWorkflow?.handleSkillSelectorInput(event);
