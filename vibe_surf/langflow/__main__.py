...
from vibe_surf.langflow.cli.progress import create_langflow_progress
from vibe_surf.langflow.initial_setup.setup import get_or_create_default_folder
from vibe_surf.langflow.logging.logger import configure, logger
from vibe_surf.langflow.main import setup_app
from vibe_surf.langflow.services.auth.utils import check_key, get_current_user_by_jwt
from vibe_surf.langflow.services.deps import get_db_service, get_settings_service, is_settings_service_initialized, session_scope
...
from vibe_surf.langflow.utils.version import fetch_latest_version, get_version_info
from vibe_surf.langflow.utils.version import is_pre_release as langflow_is_pre_release
    ...
    *,
    ...
    if platform.system() == "Windows":
        ...
        with progress.step(6):
            ...
            from vibe_surf.langflow.server import LangflowApplication
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    # Use vibe_surf.langflow.utils.version to get the version info
    ...
    telemetry_text = (
        (
            "We collect anonymous usage data to improve vibe_surf.langflow.\n"
        ...
        if os.getenv("DO_NOT_TRACK", os.getenv("LANGFLOW_DO_NOT_TRACK", "False")).lower() != "true"
            ...
            "We are [bold]not[/bold] collecting anonymous usage data to improve vibe_surf.langflow.\n"
...
async def _create_superuser(username: str, password: str, auth_token: str | None):
    ...
    from vibe_surf.langflow.services.database.models.user.crud import get_all_superusers
    ...
    async with session_scope() as session:
        ...
        if await create_super_user(db=session, username=username, password=password):
            ...
            from vibe_surf.langflow.services.database.models.user.model import User
...
def copy_db() -> None:
    ...
    This function copies the 'vibe_surf.langflow.db' and 'langflow-pre.db' files from the cache directory to the current
    directory.
    ...
    db_path = cache_dir / "vibe_surf.langflow.db"
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from vibe_surf.langflow.services.database.models.api_key.crud import create_api_key, delete_api_key
            from vibe_surf.langflow.services.database.models.api_key.model import ApiKey, ApiKeyCreate
