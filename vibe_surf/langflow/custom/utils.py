...
from vibe_surf.langflow.custom.dependency_analyzer import analyze_component_dependencies
from vibe_surf.langflow.custom.directory_reader.utils import (
    abuild_custom_component_list_from_path,
    build_custom_component_list_from_path,
    merge_nested_dicts_with_renaming,
)
from vibe_surf.langflow.custom.eval import eval_custom_component_code
from vibe_surf.langflow.custom.schema import MissingDefault
from vibe_surf.langflow.field_typing.range_spec import RangeSpec
from vibe_surf.langflow.helpers.custom import format_type
...
from vibe_surf.langflow.template.frontend_node.custom_components import ComponentFrontendNode, CustomComponentFrontendNode
from vibe_surf.langflow.type_extraction.type_extraction import extract_inner_type
from vibe_surf.langflow.utils import validate
from vibe_surf.langflow.utils.util import get_base_classes
...
async def get_single_component_dict(component_type: str, component_name: str, components_paths: list[str]):
    ...
    for base_path in components_paths:
        ...
        if module_path.exists():
            ...
            module_name = f"vibe_surf.langflow.components.{component_type}.{component_name}"
            try:
...
async def load_custom_component(component_name: str, components_paths: list[str]):
    ...
    """
    from vibe_surf.langflow.interface.custom_component import get_custom_component_from_name
                ...
                for category_dir in base_path.iterdir():
                    if category_dir.is_dir():
                        ...
                        if component_file.exists():
                            # Try to import the module
                            module_name = f"vibe_surf.langflow.components.{category_dir.name}.{component_name}"
                            try:
                                import importlib.util
