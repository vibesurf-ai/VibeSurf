...
from vibe_surf.langflow.custom.utils import abuild_custom_components, create_component_template
from vibe_surf.langflow.logging.logger import logger
from vibe_surf.langflow.services.settings.base import BASE_COMPONENTS_PATH
...
async def import_langflow_components():
    ...
    Scans the `vibe_surf.langflow.components` package and its submodules in parallel, instantiates classes that are subclasses
    ...
    modules_dict = {}
    try:
        import vibe_surf.langflow.components as components_pkg
    except ImportError as e:
        await logger.aerror(f"Failed to import vibe_surf.langflow.components package: {e}", exc_info=True)
...
def _process_single_module(modname: str) -> tuple[str, dict] | None:
    ...
    # Extract the top-level subpackage name after "vibe_surf.langflow.components."
    # e.g., "vibe_surf.langflow.components.Notion.add_content_to_page" -> "Notion"
...
async def load_single_component(component_type: str, component_name: str, components_paths: list[str]):
    ...
    from vibe_surf.langflow.custom.utils import get_single_component_dict
...
def get_all_components(components_paths, *, as_dict=False):
    """Get all components names combining native and custom components."""
    # Import here to avoid circular imports
    from vibe_surf.langflow.custom.utils import build_custom_components
