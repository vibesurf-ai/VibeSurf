...
class XiaoHongShuApiClient:
    ...
    def __init__(self, browser_session: AgentBrowserSession, timeout: int = 60, proxy: Optional[str] = None):
        ...
        Args:
            browser_session: Browser session for authentication and cookie management
    ...
    async def _extract_browser_cookies(self) -> Dict[str, str]:
        """Extract cookies from browser session, filtering only XiaoHongShu cookies"""
        try:
            web_cookies = await self.browser_session._cdp_get_cookies()
            ...
            logger.error(f"Failed to extract cookies from browser session: {e}")
            return {}

    async def _prepare_request_headers(self, endpoint: str, payload: Optional[Dict] = None) -> Dict[str, str]:
        """
        Prepare request headers with signature
        ...
        try:
            ...
            # Execute browser JS to get encryption parameters
            ...
            if encrypt_result:
                ...
                signature_headers = create_signature_headers(
                    a1=self.session_cookies.get('a1', ''),
                    b1=b1_storage or '',
                    x_s=encrypt_result.get('X-s', ''),
                    x_t=str(encrypt_result.get('X-t', ''))
                )
                
                self.default_headers.update(signature_headers)
                
        except Exception as e:
            logger.warning(f"Failed to prepare signature headers: {e}")
    ...
    async def _make_request(self, method: str, url: str, **kwargs) -> Union[str, Dict]:
        ...
        if not self.session_cookies:
            await self.browser_session.navigate_to_url("https://www.xiaohongshu.com", new_tab=False)
            raise AuthenticationError("No valid cookies found. Please login to xiaohongshu.com first.")
    ...
    async def verify_authentication(self) -> bool:
        """
        Verify if authentication state is valid
        
        Returns:
            True if authenticated, False otherwise
        """
        logger.info("Verifying RedBook authentication state...")
        try:
            search_result = await self.search_content_by_keyword(keyword="browser-use webui")
            self.is_authenticated = bool(search_result.get("items"))
